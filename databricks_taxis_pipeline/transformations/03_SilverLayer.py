...
def silver_trips():
    ...
    df = df.withColumn('trip_status_final', 
                    when( ((col("payment_method") == "CARD") & (col("trip_status") == "SUCCESS")),"ONLINE_SUCCESS")
                    .when( ((col("payment_method") == "CARD") & (col("trip_status") == "FAILED")),"ONLINE_FAILED")
                    .when( ((col("payment_method") == "CARD") & (col("trip_status") == "PENDING")),"ONLINE_PENDING")
                    .when( ((col("payment_method") != "CARD") & (col("trip_status") == "SUCCESS")),"OFFLINE_SUCCESS")
                    .when( ((col("payment_method") != "CARD") & (col("trip_status") == "FAILED")),"OFFLINE_FAILED")
                    .when( ((col("payment_method") != "CARD") & (col("trip_status") == "PENDING")),"ONLINE_PENDING")
                    .otherwise("INVALID")
                    ) 
    df = df.withColumn('modified_at',current_timestamp())
